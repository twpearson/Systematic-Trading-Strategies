...
stock_data_end <- stock_data_end %>%
  dplyr::select(symbol, annual_return)

# Identify the top 10 stocks in terms of total annual return
top_10_stocks <- stock_data_end %>%
  dplyr::top_n(10, annual_return) %>%
  dplyr::arrange(desc(annual_return ))

# Identify the bottom 10 stocks in terms of total annual return
bottom_10_stocks <- stock_data_end %>%
  dplyr::top_n(-10, annual_return) %>%
  dplyr::arrange(annual_return)

top_10_stocks$annual_return <- format(round(top_10_stocks$annual_return, digits = 1))

bottom_10_stocks$annual_return <- format(round(bottom_10_stocks$annual_return, digits = 1))
...
stock_data_end <- stock_data_end %>%
  dplyr::select(symbol, annual_return)
sectors <- dplyr::left_join(sectors, stock_data_end, by = "symbol")


stock_data_end <- dplyr::left_join(stock_data_end, sectors, by = c("symbol", "annual_return"))



mean_return_df <- sectors %>%
  dplyr::group_by(sector) %>%
  dplyr::summarize(mean_return = mean(annual_return))
...
stock_data<-stock_data %>%
  dplyr::mutate(daily_returns = ((close-open)/open))
...
d_r_p <-
  ggplot(stock_data, aes(date, daily_returns)) + 
  geom_line(size = 1, color = 'red') + 
  labs(y="daily returns in %", x="Date")+
  ggtitle("Daily Returns")
print( d_r_p )

c_r_p <-
  ggplot(stock_data, aes(date, cumm_ret)) + 
  geom_line(size = 1, color = 'green') + 
  labs(y="cumm returns in %", x="Date") +
  ggtitle("Cummulative Returns")
print( c_r_p )

m_r_p <-
  ggplot(stock_data, aes(date, max_cumm_return)) +
  geom_line(size = 1, color = 'blue') +
  labs(y="max cumm returns in %", x="Date") +
  ggtitle("Max Cummulative Returns")
print( m_r_p)

p4 <- 
  ggplot(stock_data, aes(x = date)) +
  geom_line(aes(y = cumm_ret, group = 1, color = "Cumulative Return")) +
  geom_line(aes(y = daily_returns, group = 1, color = "Daily Return"), linewidth = 0.7) +
  geom_line(aes(y = max_cumm_return, group = 1, color = "Max Cumulative Return"), linetype = "dashed")+
  labs(y="returns in %", x="Date") + 
